...
fn test_default_config_generation() {
    ...
    let parsed_config = AgentConfig::from_toml_str(&toml_str)
        .expect("Generated TOML should be parseable");

    // Verify roundtrip consistency
    assert_eq!(config.workspace_path, parsed_config.workspace_path);
    assert_eq!(config.setup_commands.len(), parsed_config.setup_commands.len());
}
...
fn test_config_generation_with_setup_commands() {
    ...
    assert!(toml_str.contains("[[setup_commands]]"), "Should contain setup_commands array");
    assert!(toml_str.contains("test_command"), "Should contain command name");
    assert!(toml_str.contains("echo"), "Should contain command");

    // Verify roundtrip
    let parsed_config = AgentConfig::from_toml_str(&toml_str)
        .expect("Should parse TOML with setup commands");

    assert_eq!(config.setup_commands.len(), parsed_config.setup_commands.len());
    assert_eq!(config.setup_commands[0].name, parsed_config.setup_commands[0].name);
    assert_eq!(config.setup_commands[0].command, parsed_config.setup_commands[0].command);
}
...
fn test_config_generation_consistency() {
    ...
    assert_eq!(toml1, toml2, "Default configs should generate identical TOML");
}
...
fn test_config_workspace_path_validation() {
    ...
    assert!(!parsed_config.workspace_path.as_os_str().is_empty(), "Workspace path should not be empty");
    assert!(Path::new(&parsed_config.workspace_path).is_absolute() ||
