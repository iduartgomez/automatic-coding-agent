...
async fn test_backup_strategy_with_failing_command() {
    ...
    assert!(result.is_ok(), "Backup strategy should allow system initialization to succeed");
}
...
async fn test_backup_strategy_with_specific_error_condition() {
    ...
    let setup_commands = vec![
        SetupCommand::new("specific_error", "sh")
            .with_args(vec!["-c".to_string(), "echo 'specific error message' >&2; exit 1".to_string()])
            .optional()
            .with_error_handler(ErrorHandler::backup(
                "conditional_backup",
    ...
    let result = AgentSystem::new(config).await;
    assert!(result.is_ok(), "Conditional backup should work when error condition is met");
}
...
async fn test_backup_strategy_no_trigger() {
    ...
    let result = AgentSystem::new(config).await;
    assert!(result.is_ok(), "System should initialize even when optional command fails without backup");
}
...
async fn test_backup_strategy_success_case() {
    ...
    let result = AgentSystem::new(config).await;
    assert!(result.is_ok(), "System should initialize successfully when primary command succeeds");
}
...
async fn test_backup_strategy_with_exit_code_condition() {
    ...
    let result = AgentSystem::new(config).await;
    assert!(result.is_ok(), "Backup should trigger for exit codes in specified range");
}
...
async fn test_backup_strategy_complex_condition() {
    ...
    let setup_commands = vec![
        SetupCommand::new("complex_condition", "sh")
            .with_args(vec!["-c".to_string(), "echo 'installation failed' >&2; exit 2".to_string()])
            .optional()
            .with_error_handler(ErrorHandler::backup(
                "complex_backup",
                OutputCondition {
    ...
    let result = AgentSystem::new(config).await;
    assert!(result.is_ok(), "Complex backup condition should work correctly");
}
...
async fn test_backup_strategy_working_directory() {
    ...
    let setup_commands = vec![
        SetupCommand::new("workdir_test", "sh")
            .with_args(vec!["-c".to_string(), "echo 'workdir error' >&2; exit 1".to_string()])
    ...
    let result = AgentSystem::new(config).await;
    assert!(result.is_ok(), "Backup strategy should work with custom working directory");
